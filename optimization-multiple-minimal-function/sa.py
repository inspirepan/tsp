...
def simulated_annealing():
    ...
    steps = 0
    # 生成初始温度
    init_temperature = generate_init_temperature()
    t = init_temperature
    # 随机生成初始解
    ...
    while low_delta_f_steps < LOW_DELTA_F_STEPS_UPPER_THRESHOLD:
        ...
        for i in range(STABLE_STEPS):
            # 产生新状态
            x_next = float(generate_next(x))
            fx_next = f(x_next)
            # 接受函数判断
            if acceptance_probability(fx - fx_next, t) > np.random.uniform(0, 1):
                x, fx = x_next, fx_next
        # 记录每次满足抽样稳定准则后的信息
...
if __name__ == '__main__':
    ...
    for i in range(EPOCHS):
        ...
        x_best, fx_best, x_history, fx_history, t_history, steps, init_temperature = simulated_annealing()
        end_time = time.time()
        # 输出结果
        ...
        stats_x_best.append(x_best)
        stats_fx_best.append(fx_best)
    ...
    print("平均值".ljust(length2, " ") + "= {:.4f}".format(np.mean(stats_fx_best)))
    print("方差".ljust(length2, " ") + "= {:.2e}".format(np.var(stats_fx_best)))
    print("均方误差".ljust(length2, " ") + "= {:.2e}".format(mse(stats_fx_best, fx_min)))
